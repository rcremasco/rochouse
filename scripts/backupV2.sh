...
#  $SCRIPTPATH/exec-chronograf.sh save 2>&1 | tee -a $LOGFILE
  ...
  sudo rsync -avrWS --exclude '/docker/influxdb2/' --exclude '/docker/influxdb/data/' --exclude '*.db' --inplace /docker /save/TOSHIBA_EXT/backup/rochouse

  writeLog "cleanup previous influx backup"
  sudo rm -f /save/TOSHIBA_EXT/backup/rochouse/backup/influx.bck/ha/*

  writeLog "saving /backup to USB key"
  sudo rsync -avrWS  --inplace /backup /save/TOSHIBA_EXT/backup/rochouse
